 ...
 *
 * The v11 driver is installed on demand into a temp directory. When that is not possible (offline CI,
 * no registry), the test **skips loudly** rather than passing quietly: a green tick for a test that
 * did not run is worse than a missing test.
 */
...
function installOldDriver(dir: string): string | null {
  try {
    execFileSync('npm', ['init', '-y'], { cwd: dir, stdio: 'ignore', timeout: 60_000 });
    execFileSync(
      'npm',
...
describe('a database written by better-sqlite3@11 opens with @13', () => {
  ...
  it('reads rows the old driver wrote, with the schema migrated forward', async () => {
    ...
    // ── Write with the OLD driver, using the pre-ADR-018 schema ────────────────────────────
    const OldDatabase = (await import(`file://${oldDriverPath.split('\\').join('/')}`)).default as
      typeof Database;
    ...
    oldDb.prepare(
      `INSERT INTO memories (id, content, memory_type, created_at, last_accessed_at, tags)
       VALUES (?, ?, 'semantic', ?, ?, ?)`,
    ).run('old-1', 'written by better-sqlite3 v11', now, now, JSON.stringify(['legacy']));
    oldDb.close();
  ...
  it('reads a WAL-mode database the old driver left behind', async () => {
    ...
    const oldDb = new OldDatabase(dbPath);
    oldDb.pragma('journal_mode = WAL');
    oldDb.exec('CREATE TABLE probe (id TEXT PRIMARY KEY, v TEXT)');
    oldDb.prepare('INSERT INTO probe VALUES (?, ?)').run('a', 'value from v11');
