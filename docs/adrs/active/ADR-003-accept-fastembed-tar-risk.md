...
obligations: 0
depends_on: [ADR-002, ADR-014]
---

# ADR-003: Accept the transitive `tar` HIGH advisory from fastembed

> **Superseded by @docs/adrs/active/ADR-017-supply-chain-baseline.md (2026-09-10).**
> The trade-off analysis below stands. Its *premise* does not: it reasons that the risk is contained
> because `fastembed` is an `optionalDependency`, but optional dependencies **are installed by
> default** — "optional" means "do not fail the install if it cannot be built". `fastembed` and its
> `tar` chain were in every consumer's tree, and the first run of the audit found **nine**
> advisories there, one critical, not the single HIGH named below.
> ADR-017 removes `fastembed` from the install tree instead of accepting the risk.
